...
class ViewController: UIViewController {
    ...
    var firstFace: VNFaceObservation?
...
//        let controlPanel = UIView()
        ...
        controlPanel.contentView.addSubview(triggerButton)
        ...
        controlPanel.contentView.addSubview(albumBtn)
...
//        faceDetectionRequest = VNDetectFaceRectanglesRequest(completionHandler: self.handleFaces) // Default
    ...
    @objc func UpdateDetectionType(_ sender: UISegmentedControl) {
...
extension ViewController: AVCapturePhotoCaptureDelegate {
    func photoOutput(_ captureOutput: AVCapturePhotoOutput,
        ...
        if let face = self.firstFace {
            let oldbounds = face.boundingBox
            let transform = CGAffineTransform(scaleX: 1, y: -1).translatedBy(x: 0, y: -(capturedImage?.size.height)!)
            let translate = CGAffineTransform.identity.scaledBy(x: (capturedImage?.size.width)!, y: (capturedImage?.size.height)!)
            let facebounds = face.boundingBox.applying(translate).applying(transform)
            
            processViewController.scaleRect = facebounds
        }
...
//        let imageHeight = self.size.height
        ...
        var rect = rect
        var x = rect.origin.x * self.scale
        var y = rect.origin.y * self.scale
        var width = rect.size.width * self.scale
        var height = rect.size.height * self.scale
